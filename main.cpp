...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        if(redraw && al_is_event_queue_empty(event_queue)) {
            ...
            al_draw_text(smallfont, al_map_rgb(128,128,0), SCREEN_W / 2, 250, ALLEGRO_ALIGN_CENTRE, "Version 1.0");
            al_draw_text(medfont, al_map_rgb_f(colr,colg,colb), SCREEN_W / 2, 420, ALLEGRO_ALIGN_CENTRE, hightxt);
            if(menu == 1) {
                al_draw_text(hugefont, al_map_rgb(196,0,0), SCREEN_W / 2, 280, ALLEGRO_ALIGN_CENTRE, "Play");
                al_draw_text(medfont, al_map_rgb(64,64,64), SCREEN_W / 2, 360, ALLEGRO_ALIGN_CENTRE, "Exit");
            }
            else {
                al_draw_text(hugefont, al_map_rgb(64,64,64), SCREEN_W / 2, 280, ALLEGRO_ALIGN_CENTRE, "Play");
                al_draw_text(medfont, al_map_rgb(196,0,0), SCREEN_W / 2, 360, ALLEGRO_ALIGN_CENTRE, "Exit");
            }
            al_flip_display();
        }
    }
    ...
    while(restart) {
        ...
        ball_x = rand() % 600 + 8;
        ball_y = 32;
        // randomize angle constant base velocity
        ball_v = 5.0;
        ball_theta = ( (rand() % 90 + 225) * ALLEGRO_PI) / 180.0;
        ball_dx = cos(ball_theta) * ball_v;
        ball_dy = sin(ball_theta) * ball_v;
        game_over = false;
        redraw = true;
        paddle_tick = 0;
            ...
            if(ev.type == ALLEGRO_EVENT_TIMER) {
                ...
                if(!game_over) {
                    ...
                    if( (ball_x + ball_dx > paddle_x - BALL_SIZE) && (ball_x + ball_dx < paddle_x + PADDLE_SIZE) && (ball_y + ball_dy > SCREEN_H - 40) ) {
                        ...
                        // new x would be off the left side of the screen
                        al_play_sample(wallhit, .75, -1, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
                        new_x = 0;
                        ball_dx = -ball_dx;
                        // check to see if it also collides with top
                        if(ball_y + ball_dy < 0) {
                            new_y = 0;
                            ball_dy = -ball_dy;
                        }
                    ...
                    else if(ball_x + ball_dx > SCREEN_W - BALL_SIZE) {
                        // new x would be off the right side of the screen
                        al_play_sample(wallhit, .75, 1, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
                        new_x = SCREEN_W - BALL_SIZE;
                        ball_dx = -ball_dx;
                        // see if it also collides up top
                        if(ball_y + ball_dy < 0) {
                            new_y = 0;
                            ball_dy = -ball_dy;
                        }
                    ...
                    else if(ball_y + ball_dy < 0) {
                        // new y would be off the top of the screen
                        al_play_sample(wallhit, .75, 0, .8, ALLEGRO_PLAYMODE_ONCE, NULL);
                        new_y = 0;
                        new_x = ball_x + ball_dx;
                        ball_dy = -ball_dy;
                    }
                    else if(ball_y + ball_dy > SCREEN_H - 40) {
                        // game over
                        game_over = true;
                        new_y = ball_y + ball_dy;
                        new_x = ball_x + ball_dx;
                    ...
                    // ball hasn't been lost, score it!
                    score++;
                }
                else {
                    // end game animation
